 ...
 *
 * Overview:
 * This program implements different graph functions
 * utilizing a linked list deque.
 *
...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    // initialize stack and add source
    collection = dequeNew();
    dequePushFront(collection, source);
    
    // search until destination is found or we run out of edges
    while (!(found == 1) && !dequeIsEmpty(collection))
    {
        // current vertex is the one most recently added to collection
        ...
        // otherwise add its neighbors to the stack
        else
        {
            for (i = 0; i < current->numNeighbors; ++i)
                if (!(current->neighbors[i]->isVisited))
                    dequePushFront(collection, current->neighbors[i]);
        }
    }
