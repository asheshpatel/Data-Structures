...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    int rght = position * 2+2;
    int smallest;

    if(rght < last)
    {
        if ((*compare)(dyGet(heap, left), dyGet(heap, rght))== -1)
            smallest = left;
        else
            smallest = rght;
        /*Indicates there are two children*/
        if((*compare)(dyGet(heap, smallest), dyGet(heap, position)) == -1)
        {
            /*if value at current position is greater than smallest child, swap and call _adjustHeap*/
...
{
    dyAdd(heap, value); /* adds to end – now need to adjust position */
    int parent;
